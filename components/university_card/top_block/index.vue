...
<script>
  ...
  export default {
    ...
    data() {
      return {
        imagePath: 'https://уникальные-программы.рф/get_image?image_path=universities/default/index.jpg&compress_percent=80&compress_width=1000&compress_height=1000'
      }
    },
    mounted() {
      this.resizeOnLoad();
    ...
    methods: {
      ...
      checkImg(titleInEnglish) {
        ...
        this.$axios.$head('/get_image', {
          ...
          .then(result => {
            this.imagePath = 'https://уникальные-программы.рф/get_image?image_path=' + path + '&compress_percent=80&compress_width=1000&compress_height=1000';
          })
          .catch(result => {
            console.log('error');
          })
